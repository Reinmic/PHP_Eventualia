...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["idEvento"]) && isset($_POST["nombreEvent"]) && isset($_POST["fechaEvent"]) && isset($_POST["lugarEvent"]) && isset($_POST["activoEvent"])) {
    ...
    $consulta = "DELETE FROM evento WHERE idEvento = ? AND idOrg = ?"; // Usamos marcadores de posición en la consulta
    $stmt = $conexion->prepare($query);
    $stmt->bind_param("ii", $idEvento, $idOrganizador);
    if ($stmt->execute()) {
...
<body>
    ...
    <table>
        ...
        while ($row = $resultado->fetch_assoc()) {
            ...
            echo "<td><a href='eliminarEventoScript.php?idEvento=" . $row['idEvento'] . "'>Eliminar</a></td>";
            echo "</tr>";
        }
        ?>
    </table>
